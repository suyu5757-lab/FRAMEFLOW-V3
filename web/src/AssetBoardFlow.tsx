...
function AssetBoardCard({ data, selected, id, positionAbsoluteX, positionAbsoluteY, onNodeClick }: NodeProps<AssetFlowNode> & { onNodeClick: AssetBoardFlowProps['onNodeClick'] }) {
  ...
  if (promptCard) {
    ...
    const selectPromptCard = (event: MouseEvent) => {
      // Let the control receive its own click first. The React Flow node
      // selection handler filters the same interactive targets below; doing
      // stopPropagation here would cancel the button's onClick entirely
      // because this handler runs during the capture phase.
      if ((event.target as HTMLElement).closest('button, textarea, input, label, details, summary')) return;
